...
//LIMB DARKENING SANS MICROLENSING
//compile with gcc -lm -o limbdarkening limbdarkening.c
//store data in ld.dat
...
double SG(double R_E,float M_b,float R_a, float R_b,float d, float y,float z)
{
	...
	c1 = 0.701;//0.701;//0.5118;
	c2 = 0.149;//0.149;//0.0525;
	c3 = 0.277;//0.277;//0.4590;
	c4 = -0.297;//-0.297;//-0.2727;
	...
	for(r=0;r<R_a;r += dr)
	{
		...
		for (theta=0;theta<2*pi;theta +=dtheta)
		{
			...
			{
				ley = fabs(y);
				ydist = fabs(ley - r*cos(theta));
			}
			if(y<0)
			{
				ley = fabs(y);
				ydist = fabs(r*cos(theta) + ley);
			}
			zdist = fabs(face - z);//determine zdist = z distance between point in integration and centre of B.
			l = sqrt(zdist*zdist + ydist*ydist);//determine between pnt in integration and centre of B.
			if(r!=0)
			{
				rnorm = r/R_a;
